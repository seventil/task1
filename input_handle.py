...
def get_initial_args():
    parser = argparse.ArgumentParser(
        description="Join students and rooms files")
    parser.add_argument("students", type=valid_path,
                        help="Path to students file")
    parser.add_argument("rooms", type=valid_path,
                        help="Path to rooms file")
    parser.add_argument("output", type=valid_type,
                        help="Type of an output file")
...
def valid_type(file_type):
    ...
    if file_type.upper() not in VALID_TYPES:
        raise argparse.ArgumentTypeError(
            "Invalid file format (choose XML or JSON)")
    else:
        return file_type
